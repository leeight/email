...
func (h UploadControllerHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "file":
		h.receiveAttachmentUpload(w, r)
	default:
		http.Error(w, "Invalid request", http.StatusBadRequest)
	}
}

func (h UploadControllerHandler) receiveAttachmentUpload(w http.ResponseWriter, r *http.Request) {
	...
	for _, fileHeaders := range r.MultipartForm.File {
		for _, fileHeader := range fileHeaders {
			...
			w.Write([]byte(fmt.Sprintf(`{"state": "SUCCESS", "url": "downloads/00000/att/%s"}`,
				path.Base(abspath))))
		}
	}
}
...
func (h UploadControllerHandler) sendServerConfig(w http.ResponseWriter, r *http.Request) {
	var config = &serverConfig{
		...
		"",
		5 * 1024 * 1024,
		[]string{".pdf", ".txt", ".doc", ".docx", ".ppt", ".pptx", ".xls", ".xlsx"},
		"file",
		"file",
		"",
	}
